...
class GridWorld2D:
	...
	def generate_rect_obstacle_at_empty_pos(self, max_obstacle_dim):
	...
	def add_rect_obstacle(self, max_obstacle_dim):

		obstacle_added = 0

		# Generate obstacle and add it to the world
		coordinates = self.generate_rect_obstacle_at_empty_pos(max_obstacle_dim)
		...
		for i in range(len(self.agent_locs)):
			...
			if self.wave_front(i)[self.agent_locs[i][0], self.agent_locs[i][1]] == WORLD['EMPTY']:
	...
	def wave_front(self, agent_index):
	...
	def set_random_agent_init_locs(self):
	...
	def take_action(self, action, agent_index):
		...
		if try_move and \
		not out_of_bounds(new_position[0], new_position[1], len(self.world), len(self.world[0])) and \
